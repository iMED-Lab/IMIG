"""
Receptive field computation utilities for prototype-based networks.
Computes the receptive field size and location at each layer of the backbone,
used to map prototype activations back to input image regions for visualization.
"""
...
def compute_layer_rf_info(layer_filter_size, layer_stride, layer_padding,
    ...
    """Compute receptive field info after one convolutional layer."""
...
def compute_rf_protoL_at_spatial_location(img_size, height_index, width_index, protoL_rf_info):
    """Compute the receptive field bounding box at a given spatial location."""
    n = protoL_rf_info[0]
    j = protoL_rf_info[1]
    r = protoL_rf_info[2]
...
def compute_rf_prototype(img_size, prototype_patch_index, protoL_rf_info):
    """Compute the receptive field for a single prototype patch."""
...
def compute_rf_prototypes(img_size, prototype_patch_indices, protoL_rf_info):
    """Compute receptive fields for a batch of prototype patches."""
    rf_prototypes = []
    for prototype_patch_index in prototype_patch_indices:
        rf_prototypes.append(compute_rf_prototype(
...
def compute_proto_layer_rf_info_v2(img_size, layer_filter_sizes, layer_strides,
    ...
    """Compute the receptive field of the prototype layer given backbone conv info."""
    ...
    for i in range(len(layer_filter_sizes)):
        rf_info = compute_layer_rf_info(
            layer_filter_size=layer_filter_sizes[i],
            layer_stride=layer_strides[i],
